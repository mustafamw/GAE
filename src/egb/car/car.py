...
class CarList(messages.Message):
    car_list = messages.MessageField(Car, 1, repeated=True)
    

class CarSignup(messages.Message):
    car_signup_key = messages.IntegerField(1, required=True)
...
class CarSignupList(messages.Message):
    car_signup_list = messages.MessageField(CarSignup, 1, repeated=True)
...
class CarSignupModel(ndb.Model):
    car =  ndb.KeyProperty(kind=CarModel)
    user = ndb.KeyProperty(kind=UserModel, required=True)
    user_id = ndb.IntegerProperty(required=True)
    provider_name = ndb.StringProperty(required=True)
    product_type = msgprop.EnumProperty(ProductType, required=True, indexed=True)
